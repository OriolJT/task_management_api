...
- Live Demo: https://your-demo.example.com (replace with your URL)
- API Docs: https://orioljt.github.io/task_management_api/

Pre-seeded test users (via Keycloak import):

- alice / alice (realm roles: user, admin)
- bob / bob (realm role: user)

Get a token directly (password grant) on a running demo:

```bash
curl -s -X POST \
  -H 'Content-Type: application/x-www-form-urlencoded' \
  -d 'client_id=task-api&grant_type=password&username=alice&password=alice' \
  https://your-keycloak.example.com/realms/task-realm/protocol/openid-connect/token
```

Try the API with the Bearer token:

```bash
curl -H "Authorization: Bearer <token>" -H 'Content-Type: application/json' \
  -d '{"name":"Demo Project"}' https://your-demo.example.com/api/projects
```

Access control demo: use Bob’s token on Alice’s project and you’ll receive 404 (not owned).
