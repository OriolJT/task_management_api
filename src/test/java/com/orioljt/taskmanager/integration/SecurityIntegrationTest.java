...
class SecurityIntegrationTest {
            ...
            post("/api/users")
                ...
                .content(
                    om.writeValueAsString(
                        new CreateUserRequest("int-user@example.com", "Password123"))))
  ...
  void userTokenCanCreateAndListOwnProjects() throws Exception {
    ...
    mvc.perform(
        ...
        .andExpect(
            header().string("Location", org.hamcrest.Matchers.matchesPattern("/api/projects/.*")));
  ...
  void adminTokenCanAccessAdminEndpointsAndUserTokenCannot() throws Exception {
    ...
    mvc.perform(
            get("/api/admin/users/{id}", u.getId()).header("Authorization", "Bearer " + adminToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(u.getId().toString()));

    mvc.perform(
            get("/api/admin/users/{id}", u.getId()).header("Authorization", "Bearer " + userToken))
  ...
  void userCannotPatchAdminUpdateUser() throws Exception {
    ...
    String userToken = "user_" + UUID.randomUUID();
    mvc.perform(
            patch("/api/admin/users/{id}", u.getId())
